...
* Edit
** code
In general I would require diagnostics (errors, warnings and other insights), autocomplete and autofixing for programming. If at all possible the diagnostics should be provided by a language server to get the most information. The autocompletion should also additionally provide auto-imports when applicable (or other related actions that should be executed after completion).
*** show line numbers
...
And let's add some keybindings to make this more useful. I am trying something new here, a bit like a cursor cluster:
*i* is the central key, *ii* will toggle the popup. *io* will cycle and *iu* will kill the popup.
Note: this are not mnemonic keybinds, but rather directional (similar to the arrow cluster).
#+begin_src emacs-lisp
  (oem/leader-key-def
